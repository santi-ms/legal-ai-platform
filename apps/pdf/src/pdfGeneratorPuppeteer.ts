...
export type GeneratePdfInput = {
  ...
  rawText: string;
  fileName?: string;
  /** Base64 data URI or URL of the firm logo to include in the PDF header */
  logoDataUri?: string;
};
...
function buildFullHtml(title: string, bodyHtml: string, logoDataUri?: string): string {
...
<body>
  <div class="doc-wrapper">
    ${logoDataUri ? `<div class="firm-logo-header"><img src="${logoDataUri}" alt="Logo del estudio" /></div>` : ""}
    ${bodyHtml}
    <div class="legal-disclaimer">
      <strong>AVISO LEGAL:</strong> El presente documento fue generado mediante la plataforma
      <strong>DocuLex</strong> como herramienta de asistencia tecnológica para la redacción de
      documentos legales. Su contenido tiene carácter informativo y de borrador preliminar. No
      constituye asesoramiento jurídico profesional ni reemplaza la intervención, revisión y
      aprobación de un abogado matriculado. DocuLex S.R.L. no asume responsabilidad por errores,
      omisiones o inexactitudes en el contenido generado, ni por su adecuación a la situación
      particular de las partes o a la normativa vigente aplicable. Antes de su firma, presentación
      ante autoridades o cualquier uso con efectos jurídicos, este documento debe ser revisado por
      un profesional del derecho habilitado.
    </div>
  </div>
</body>
</html>`;
...
export async function generatePdfFromContract({
  ...
  const fullHtml = buildFullHtml(title, bodyHtml, logoDataUri);
