...
6. Jurisdicción: el sistema opera únicamente en las siguientes jurisdicciones. Usá EXACTAMENTE uno de estos valores:
   "corrientes_capital" → Corrientes (capital o cualquier ciudad de la provincia de Corrientes)
   "posadas_misiones"   → Misiones (provincia, Posadas, Oberá, Eldorado, o cualquier ciudad misionera)
   Si el usuario menciona otra provincia o no menciona jurisdicción, usá "corrientes_capital" por defecto.
   Si el usuario menciona explícitamente Misiones o alguna ciudad misionera, usá "posadas_misiones".
...
Cuando tenés TODA la información necesaria para el tipo detectado:
{
  ...
  "extractedData": {
    "documentType": "legal_notice",
    "jurisdiction": "corrientes_capital",
    "remitente_nombre": "...",
    "remitente_doc": "-",
...
export async function registerChatRoutes(app: FastifyInstance) {
  ...
  app.post("/documents/chat", {
    config: {
      rateLimit: {
        max: 60,
        timeWindow: 60 * 1000, // 60 mensajes de chat por minuto por IP
        errorResponseBuilder: () => ({
          ok: false,
          error: "too_many_requests",
          message: "Demasiados mensajes seguidos. Esperá un momento.",
        }),
      },
    },
  }, async (request, reply) => {
  ...
  // Nota: POST /documents/:id/ask está registrado en routes.documents.ts
  // (con rate limiting y aislamiento de tenant). No duplicar aquí.
}
