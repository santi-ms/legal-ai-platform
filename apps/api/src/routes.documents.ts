...
// ─── DB-backed job store for async document generation ───────────────────────
// Persists generation jobs in PostgreSQL via the GenerationJob model.
// Survives server restarts and works across multiple instances.
// Prevents Railway/Vercel 60-second timeouts on Claude generation (~60-90s).

// Purge GenerationJob rows older than 30 minutes once per hour
setInterval(async () => {
  try {
...
export async function registerDocumentRoutes(app: FastifyInstance) {
  ...
  app.get("/documents/stats", async (request, reply) => {
    try {
      ...
      const [allDocs, allDocsWithMeta, totalClients, newClientsThisMonth, expedientesActivos, vencimientosUrgentes, expedientesByMateria, activeShares, totalAnalyses, costTotal, costThisMonth] = await Promise.all([
        ...
        // Links de compartición activos (no expirados, no revocados)
        prisma.documentShare.count({ where: { tenantId, status: "active", expiresAt: { gt: now } } }),
        // Total análisis de contratos del tenant
        prisma.contractAnalysis.count({ where: { tenantId } }),
        // Total AI cost (USD) all time
        prisma.document.aggregate({ where: { tenantId }, _sum: { costUsd: true } }),
        // AI cost (USD) this month
        prisma.document.aggregate({ where: { tenantId, createdAt: { gte: startOfThisMonth } }, _sum: { costUsd: true } }),
      ]);
      ...
      return reply.send({
        ...
        activeShares,
        totalAnalyses,
        aiCostUsdTotal: costTotal._sum.costUsd ?? 0,
        aiCostUsdThisMonth: costThisMonth._sum.costUsd ?? 0,
      });
    } catch (err) {
  ...
  app.get("/documents", async (request, reply) => {
    try {
      ...
      // Búsqueda de texto (en type, jurisdiccion y contenido del documento)
      if (query) {
        where.OR = [
      ...
      // Función para extraer un snippet del texto alrededor de la primera coincidencia
      function extractSnippet(text: string | null | undefined, q: string, maxLen = 160): string | null {
      ...
      return reply.send({
        ...
        items: documents.map((doc: DocumentWithVersion) => {
          ...
          // Calcular snippet solo cuando hay búsqueda de texto
          let matchSnippet: string | null = null;
          if (query && version) {
            const content = (version as any).editedContent || version.rawText;
            matchSnippet = extractSnippet(content, query);
          }
          return {
            id: doc.id,
            type: doc.type,
  ...
  app.post("/documents/generate", {
    config: {
      rateLimit: {
        max: 10,
        timeWindow: 60 * 1000, // 10 generaciones por minuto por IP
        errorResponseBuilder: () => ({
          ok: false,
          error: "too_many_requests",
          message: "Demasiadas solicitudes de generación. Esperá un momento antes de intentar de nuevo.",
        }),
      },
    },
  }, async (request, reply) => {
    try {
      ...
      await prisma.generationJob.create({ data: { id: jobId, tenantId, status: "pending" } });
      ...
      // 6.5️⃣ Obtener logo del estudio (si existe)
      let tenantLogoDataUri: string | undefined;
      if (tenantId !== "demo-tenant") {
        try {
          const tenantWithLogo = await (prisma as any).tenant.findUnique({
            where: { id: tenantId },
            select: { logoUrl: true },
          });
          if (tenantWithLogo?.logoUrl) {
            tenantLogoDataUri = tenantWithLogo.logoUrl;
          }
        } catch (_err) {
          ...
          const pdfResponse = await fetch(`${pdfServiceUrl}/pdf/generate`, {
            ...
            body: JSON.stringify({
              title: ({
                ...
                cease_desist: "CARTA DE CESE Y DESISTA",
                employment_contract: "CONTRATO DE TRABAJO",
                power_of_attorney: "PODER NOTARIAL",
              } as Record<string, string>)[jobDocumentType] ?? jobDocumentType.replace(/_/g, " ").toUpperCase(),
              rawText: contrato,
              fileName: fileName,
              ...(tenantLogoDataUri ? { logoDataUri: tenantLogoDataUri } : {}),
            }),
          });
      ...
      if (pdfGenerated) {
        await prisma.documentVersion.update({
          ...
          await prisma.generationJob.update({
            ...
            data: {
              status: "done",
              // Cast to any: Prisma's InputJsonValue doesn't accept typed arrays
              // like GenerationWarning[] — they are serializable JSON but TS is strict.
              result: {
                ok: true,
          ...
          }).catch(() => { /* non-critical — job may not exist if create failed */ });
        }
      }); // end setImmediate
  ...
  app.get("/documents/jobs/:jobId", async (request, reply) => {
    // Mirror the generate endpoint's auth strategy:
    // - In production: require a valid JWT
    // - In dev/test: fall through unauthenticated (demo-tenant)
    const user = getUserFromRequest(request);
    if (!user && process.env.NODE_ENV === "production") {
    ...
    // In production, verify the job belongs to the caller's tenant
    if (job && user?.tenantId && job.tenantId !== user.tenantId) {
    ...
    // status === "done"
    return reply.send({ ok: true, status: "done", ...(job.result as Record<string, unknown>) });
  });
  ...
  app.delete("/documents/:id", async (request, reply) => {
    try {
      ...
      // Eliminar (cascade elimina versiones). Defense in depth: deleteMany
      // con filtro por tenantId aunque ya chequeamos arriba.
  ...
  app.patch("/documents/:id", async (request, reply) => {
    try {
      ...
      // Actualizar. Defense in depth: updateMany con filtro por tenantId.
  ...
  app.patch("/documents/:id/annotations/:annotationId", async (request, reply) => {
    ...
    try {
      // Defense in depth: updateMany filtra por tenantId + documentId para
      // que ningún tenant ajeno pueda tocar este recurso aunque conozca el UUID.
      const result = await (prisma as any).documentAnnotation.updateMany({
        where: {
          id: parsed.data.annotationId,
          documentId: parsed.data.id,
          tenantId: user.tenantId,
        },
        data: body.data,
      ...
      const updated = await (prisma as any).documentAnnotation.findFirst({
        where: { id: parsed.data.annotationId, tenantId: user.tenantId },
        include: { author: { select: { id: true, name: true, email: true } } },
      });
  ...
  app.delete("/documents/:id/annotations/:annotationId", async (request, reply) => {
    ...
    try {
      // Defense in depth: el deleteMany filtra por tenantId + documentId
      // directamente, garantizando que ningún tenant ajeno pueda borrar este
      // recurso aunque conozca el UUID.
          ...
          const pdfResponse = await fetch(`${pdfServiceUrl}/pdf/generate`, {
            ...
            body: JSON.stringify({
              title: ({
                ...
                employment_contract: "CONTRATO DE TRABAJO",
                power_of_attorney: "PODER NOTARIAL",
              } as Record<string, string>)[document.type] ?? document.type.replace(/_/g, " ").toUpperCase(),
  ...
  // ==========================================
  // Permite hacer preguntas a la IA sobre un documento específico.

  app.post("/documents/:id/ask", {
    config: {
      rateLimit: {
        max: 30,
        timeWindow: 60 * 1000, // 30 preguntas por minuto por IP
        errorResponseBuilder: () => ({
          ok: false,
          error: "too_many_requests",
          message: "Demasiadas preguntas seguidas. Esperá un momento.",
        }),
      },
    },
  }, async (request, reply) => {
    const AskBodySchema = z.object({ question: z.string().min(1).max(2000) });
    try {
      const user = requireAuth(request);
      if (!user.tenantId) {
        return reply.status(403).send({ ok: false, error: "TENANT_REQUIRED" });
      ...
      // Verificar que el plan incluye chatIA
      const { getPlanForTenant } = await import("./routes.billing.js");
      const { plan } = await getPlanForTenant(user.tenantId);
      const features = (plan as any)?.features ?? {};
      if (features.chatIA === false) {
        return reply.status(403).send({
          ok: false,
          error: "PLAN_FEATURE_UNAVAILABLE",
          message: "Las preguntas a la IA no están disponibles en tu plan actual. Actualizá tu plan para usar esta función.",
        });
      }
      ...
      const doc = await prisma.document.findFirst({
        ...
        include: {
          versions: {
            orderBy: { createdAt: "desc" },
            take: 1,
            select: { rawText: true, editedContent: true },
          },
        },
      });
      ...
      const content = doc.versions?.[0]?.editedContent || doc.versions?.[0]?.rawText || "";
      ...
      if (err.message === "UNAUTHORIZED") return reply.status(401).send({ ok: false, error: "UNAUTHORIZED" });
      app.log.error(err, "[api] Error in /documents/:id/ask");
      return reply.status(500).send({ ok: false, message: "Error al procesar la pregunta" });
    }
  });
  ...
  // ==========================================
  // Genera sugerencias de revisión con IA para un documento específico.
  // Devuelve un array de cambios sugeridos con texto original, sugerido y explicación.

  app.post("/documents/:id/revisions", {
    config: {
      rateLimit: {
        max: 5,
        timeWindow: 60 * 1000, // 5 revisiones por minuto por IP (Claude Sonnet, costoso)
        errorResponseBuilder: () => ({
          ok: false,
          error: "too_many_requests",
          message: "Demasiadas solicitudes de revisión. Esperá un momento.",
        }),
      },
    },
  }, async (request, reply) => {
    try {
      ...
      if (features.chatIA === false) {
        return reply.status(403).send({
          ok: false,
          error: "PLAN_FEATURE_UNAVAILABLE",
          message: "Las sugerencias de revisión no están disponibles en tu plan actual. Actualizá tu plan para usar esta función.",
        });
      }
      ...
      const response = await anthropic.messages.create({
        model: "claude-sonnet-4-6",
        max_tokens: 2048,
        system: `Sos un abogado revisor experto en derecho argentino. Tu tarea es analizar documentos legales y sugerir mejoras concretas y específicas.

Vas a recibir un documento legal y debés devolver EXACTAMENTE entre 3 y 5 sugerencias de mejora en formato JSON.

Reglas importantes:
- El campo "original" debe ser una cita textual EXACTA del documento (máximo 120 caracteres). Preferí frases cortas y específicas.
- El campo "suggested" debe ser la versión mejorada de esa frase (máximo 150 caracteres).
- El campo "section" es el nombre de la cláusula o sección donde aparece el texto.
- El campo "explanation" es una explicación breve del por qué mejora el texto (máximo 200 caracteres).
- Enfocate en: precisión jurídica, ambigüedades, términos que conviene aclarar, plazos o montos que deben ser más específicos.
- Solo citá texto que realmente existe en el documento.

Respondé ÚNICAMENTE con un JSON válido, sin texto adicional, con esta estructura exacta:
[
  {
    "id": "rev-1",
    "section": "nombre de la cláusula",
    "original": "texto exacto del documento",
    "suggested": "texto mejorado propuesto",
    "explanation": "explicación breve de la mejora"
  }
]`,
        messages: [{
          role: "user",
          content: `Analizá este documento y devolvé entre 3 y 5 sugerencias de mejora en JSON:\n\n${content.slice(0, 14000)}`,
        }],
      });
      ...
      if (err.message === "UNAUTHORIZED") return reply.status(401).send({ ok: false, error: "UNAUTHORIZED" });
      app.log.error(err, "[api] Error in /documents/:id/revisions");
      return reply.status(500).send({ ok: false, message: "Error al generar revisiones" });
    }
  });
  ...
  // ==========================================
  // Genera y devuelve el documento en formato Word (.docx).
  ...
  app.get("/documents/:id/docx", async (request, reply) => {
    try {
      ...
      const documentTitle = docTypeLabels[doc.type] || doc.type || "Documento Legal";

      // Parsear el contenido en párrafos y secciones
      ...
      // Jurisdicción y fecha
      if (doc.jurisdiccion) {
        const jurisdiccionLabels: Record<string, string> = {
          corrientes_capital: "Corrientes Capital, Argentina",
          posadas_misiones: "Posadas, Misiones, Argentina",
        };
        children.push(
          new Paragraph({
            children: [
              new TextRun({
                text: jurisdiccionLabels[doc.jurisdiccion] || doc.jurisdiccion,
                italics: true,
                color: "666666",
                size: 20,
              }),
      ...
      for (const line of lines) {
        ...
        // Detectar encabezados numerados (ej: "1. PARTES", "CLÁUSULA I:", "ARTÍCULO 1.-")
        const isHeading = /^(\d+[\.\-]|cláusula\s+\w+|artículo\s+\d+|[IVX]+[\.\-])/i.test(trimmed);
        if (isHeading) {
          children.push(
            new Paragraph({
              text: trimmed,
      ...
      children.push(
        ...
        new Paragraph({
          children: [
            new TextRun({ text: "Firma del Cliente", size: 20, color: "888888" }),
            new TextRun({ text: "                                      " }),
            new TextRun({ text: "Firma del Prestador", size: 20, color: "888888" }),
          ],
          spacing: { after: 400 },
        }),
        new Paragraph({
          children: [
            new TextRun({
              text: "Documento generado por DocuLex — Plataforma de documentos legales con IA. Este documento tiene carácter de borrador.",
              italics: true,
              color: "999999",
              size: 16,
            }),
      ...
      const safeTitle = documentTitle.replace(/[^a-zA-Z0-9\s]/g, "").replace(/\s+/g, "_");
      ...
      if (err.message === "UNAUTHORIZED") return reply.status(401).send({ ok: false, error: "UNAUTHORIZED" });
      app.log.error(err, "[api] Error in /documents/:id/docx");
      return reply.status(500).send({ ok: false, message: "Error al generar el archivo Word" });
    }
  });
}
