...
const SHORTCUTS: { category: string; items: Shortcut[] }[] = [
  ...
  },
  {
    ...
    items: [
      { keys: ["G", "D"], description: "Panel de control" },
      { keys: ["G", "L"], description: "Mis documentos" },
      { keys: ["G", "C"], description: "Clientes" },
      { keys: ["G", "E"], description: "Expedientes" },
      { keys: ["G", "V"], description: "Vencimientos" },
      { keys: ["G", "F"], description: "Finanzas" },
      { keys: ["G", "K"], description: "Calendario" },
      { keys: ["G", "A"], description: "Analytics" },
      { keys: ["G", "J"], description: "Doku Juris" },
...
export function KeyboardShortcutsModal({ open, onClose }: KeyboardShortcutsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      {/* Modal */}
      <div className="relative z-10 bg-white dark:bg-slate-900 rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-md border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-150">
        {/* Header */}
